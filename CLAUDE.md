...
4. **Retrieve** — `retrieval_service.retrieve_candidates(question, fallback_game=...)` runs vector search and keyword search *concurrently* (`ThreadPoolExecutor`) since each Chroma Cloud round trip costs ~200-250ms; merges and dedupes results. `game_detector.detect_games()` returns *every* game the question names, not just one — a question naming two games (e.g. "compare Tekken 8 and PUBG's rules") scopes the Chroma query to both via `$in`, rather than silently collapsing to whichever one has more mentions. `fallback_game` carries the previous turn's detected game forward so follow-up questions that don't name a game (e.g. "what about substitutes?") stay scoped instead of searching unscoped across every game. If a question names no game *and* there's no `fallback_game` either, `ChatView` asks the user which game they mean instead of running an unscoped search — see `docs/EDGE_CASES.md`.
