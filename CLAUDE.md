...
- `jsconfig.json`'s `include` covers `src/components/**/*.js`, `src/pages/**/*.jsx`, and `src/Layout.jsx` — it explicitly excludes `src/components/ui`, `src/lib`, and `src/api`, so `npm run typecheck` does not type-check those directories.
