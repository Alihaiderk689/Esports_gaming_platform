...
3. **Email-verified registration** (`core/models.py:PendingRegistration` → `core/views.py:VerifyEmailView`) — signup does **not** create a `User`. It creates a `PendingRegistration` row (hashed password, not plaintext) and emails a 6-digit one-time code (`core/otp.py`; only its hash is stored, with a 10-minute expiry and a 5-attempt lockout). Only when that code is submitted correctly to `POST /api/auth/verify-email/` does `VerifyEmailView` create the real `User` (and `Organizer`, if applicable) inside one transaction, then delete the pending row. No correct code, no account — this is enforced structurally, not by a flag that could be bypassed.
...
| **Brevo** | Verification/reset/announcement/reschedule/tournament-win emails, via Brevo's transactional email HTTP API (not SMTP) | `core/email_backend.py:BrevoAPIBackend` (a Django `EMAIL_BACKEND`), called by `core/emails.py`/`tourny_regist/emails.py` through ordinary `django.core.mail.send_mail()` | A non-2xx response raises `RuntimeError`, logged first; callers that loop over many recipients (`send_announcement_emails`/`send_reschedule_email`) catch per-recipient so one bad address doesn't stop the rest. Dev default (`BREVO_API_KEY` unset) falls back to Django's console backend (prints to stdout, no real send). Production must set `BREVO_API_KEY` — see the security checklist. |
